...
export class AssistPage {
  ...
  launchChat(member_id) {
    var loading = this.loadingCtrl.create({content:'Loading chat...'});
    loading.present().then(()=>{
      ...
      CCCometChat.launchCometChatWithID(isFullScreen, member_id, false, true, function success(data) {
        ...
        var x = function userInfoCallback(res){
          console.log('res',res);
        };
        loading.dismiss();
      }, function error(data) {
  ...
  respondToRequest(item) {
    console.log('respondToRequest', item);
    if (item.connected) {
      this.launchChat(item.member_id);
    }
    else {
      this.provider.getData('ID').then(id => {
